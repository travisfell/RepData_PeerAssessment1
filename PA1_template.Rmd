...
```
Second, impute data to those NA values. We will use the average of the given interval to impute NA values for that same interval. To do this, we need to calculate the average per interval steps excluding NA values. We also need to give the results set meaningful column names. 
```{r}
